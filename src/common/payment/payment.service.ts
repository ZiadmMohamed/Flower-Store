...
async webhook(req:Request){
        ...
        const endpointsecret=process.env.stripe_webhook_signin_secret  as string
          ...
          if (event.type != 'checkout.session.completed') {
            await   this.orderRepo.updateOne({_id:event.data.object["metadata"].orderId,orderStatus:ORDER_STATUS.PENDING},{orderStatus:ORDER_STATUS.CANCELLED,rejectedReason:"fail to pay"})

           throw new BadRequestException("fail to pay")

          }
          const order=await this.orderRepo.findOne({filters:{id:event.data.object["metadata"]?.orderId}})
          if(!order){
            throw new NotFoundException("order id is not avaialbe")
          }
          await this.confirmPaymenIntent(order.intentId)
        await   this.orderRepo.updateOne({_id:event.data.object.metadata?.orderId,orderStatus:ORDER_STATUS.PENDING},{orderStatus:ORDER_STATUS.CONFIRMED,paidAt:new Date(Date.now())})
