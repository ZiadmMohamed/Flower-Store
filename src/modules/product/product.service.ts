...
if(!categoryExist){
  ...
  async updateProduct(param:ProductIdDTO,user:UserDocument,body:updateProductDTO,file?:Express.Multer.File ){
    const{productId}=param
    const categoryExist=await this.categoryRepo.findCategoryById(body.categoryId)
if(!categoryExist){
  throw new BadRequestException("category is not exist")
}
...
image=await this.cloudservice.uploadFile(file,{folder:`${process.env.app_name}/product/${product.categoryId}/${product.folderId}`})    
  }
  let finalPrice:number
if(body.discountAmount || body.originalPrice){
finalPrice=   Number(body.originalPrice || product.originalPrice) -
        (Number(body.originalPrice || product.originalPrice) * Number(body.discountAmount || product.discountAmount)) / 100;
}
const updatedProduct=await this.ProductRepo.updateOne({_id:productId},{...body,finalPrice,image})
